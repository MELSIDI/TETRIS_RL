...
class Tetris:
    ...
    def get_state(self):
        # Le piece space = 4x10 pour les pieces + 1x10 pour la separation 
        pieces_space = [[0 for _ in range(MATRIX_WIDTH)] for _ in range(TETRIMINO_LEN + 1)]

        # Ajouter le tetrimino actuelle et prochaine au space pieces
        current_tetrimino_matrix = self.current_tetrimino.value[0]
        next_tetrimino_matrix = self.next_tetrimino.value[0]
        next_start = MATRIX_WIDTH - TETRIMINO_LEN
        for y in range(TETRIMINO_LEN):
            for x in range(TETRIMINO_LEN):
                pieces_space[y][x] = current_tetrimino_matrix[y][x]
                pieces_space[y][x + next_start] = next_tetrimino_matrix[y][x]

        # On recupere le full state
        full_state = self.get_full_state()

        # On ajoute le full state 
        state = [row[:] for row in pieces_space]
        for row in full_state:
            state.append(row[:])

        # pour eviter davoir des representation d'etat differente on retourne les deux (Prévention contre la gravité)
    ...
    def set_state(self, action):
        ...
        if collision:
            if action in (Action.SOFTDROP, Action.HARDDROP):
                ...
                tetrimino_matrix = self.get_tetrimino_matrix()
                for y in range(TETRIMINO_LEN):
                    for x in range(TETRIMINO_LEN):
    ...
    def reward_shaping(self):
        ...
        pour le guider vers l'objectif final 
        avant même qu'il ne réussisse à détruire une ligne complète.
        """
        # Trouvons l'index de la ligne la plus basse de la piece :
        #  le pied du Tetrimino
        bottom_y_offset = 0
        current_tetrimino_matrix = self.get_tetrimino_matrix()
        for row_indx in range(TETRIMINO_LEN):
            if 1 in current_tetrimino_matrix:
                bottom_y_offset = row_indx

        # L'index de la ligne la plus dans les coordonnées de la matrice
        bottom_y = self.current_y + bottom_y_offset

        shaped_reward = 0

        if bottom_y < MATRIX_HEIGHT:
            shaped_reward = (sum(self.matrix[bottom_y]) / 10.0) * 3.0

        # Appliquer une penalité = au nombre de trous crer par l'agent en deposant 
        # Sa piece
        hole_malus = 0

        for col in range(TETRIMINO_LEN):
            # Trouver le bloc le plus bas dans la colonne actuelle:
            bottom_row = -1
            for row in range(TETRIMINO_LEN):
                if current_tetrimino_matrix[row][col] == 1:
                    bottom_row = row

            # En presence d'un bloc
            if bottom_row != -1:
                board_x = self.current_x + col
                board_y = self.current_y + bottom_row

                # On check si il a pas creer de trous
                for check_y in range(board_y + 1, MATRIX_HEIGHT):
                    if 0 <= board_x < MATRIX_WIDTH:
                        if self.matrix[check_y][board_x] == 0:
                            hole_malus -= 1.0 # -1 pour le trou detecté
                        else:
                            pass
